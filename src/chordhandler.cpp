...
{   
    ...
    for (QString key : chords.keys()) {
        ...
        QList<Score> scores = GuidoService::gmnToScores(value);

        Song song;
        song.name = key;
        song.gmn = value;
        song.voices.insert(UNKNOWN_HAND, scores);

        QByteArray scorePitchSequence = ScoreService::scoresToPitchSequence(scores);
        QByteArray scoreIntervalSequence = ScoreService::scoresToIntervalSequence(scores);

        MatchingItem item(song, scorePitchSequence, scoreIntervalSequence);
...
void ChordHandler::matchNoteOffEvent(NoteOffEvent noteOff) {
    for (NoteOnEvent noteOnEvent : noteOnEvents) {
        if (noteOnEvent.getNote() == noteOff.getNote()) {
            noteOnEvents.removeAll(noteOnEvent);
            break;
        }
    }
...
void ChordHandler::match() {
    ...
    for (QList<MatchingItem>::iterator i = matchingItems.begin(); i != matchingItems.end(); ++i) {
        ...
        item.quality = MatchingService::getChordQuality(item.pitchAlignment);
        *i = item;
    }
    ...
    emit chordRecognized(bestMatchingItem);
}
